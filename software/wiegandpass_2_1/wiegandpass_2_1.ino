...
/*--------------------------------------------------------*/   
   ...
   //записываем двоичный код разделяя по байтам + в разворачиваем массив обратно
    ...
    if (eeprom_find_same(byte1, byte2, byte3, byte4, byte5) and button_flag == 0 and mastercard_flag == 0 and delete_flag == 0){ //открываем шкаф, если конечно кнопка не была нажата (и другие флаги не стоят)        
    ...
    if (delete_flag == 1){
      ...
      if ((byte1 == EEPROM.read(0) and byte2 == EEPROM.read(1) and byte3 == EEPROM.read(2) and byte4 == EEPROM.read(3) and byte5 == EEPROM.read(4)) or (byte1 == EEPROM.read(5) and byte2 == EEPROM.read(6) and byte3 == EEPROM.read(7) and byte4 == EEPROM.read(8) and byte5 == EEPROM.read(9))){
        clear_eeprom();
      }
    ...
    if (button_flag == 1) { //кнопка была нажата перед тем как приложили карту?
      ...
      else if (eeprom_5bytes_free(0) or eeprom_5bytes_free(5)){ //а если карточка не подошла, ибо там их вообще нет?
        new_card_write(byte1, byte2, byte3, byte4, byte5);
        }
...
/*-------------Проверка кнопки на долгое нажатие-----------------*/ 
  ...
  if (eeprom_last_adress()==0) { //если память пустая, то хорошая идея открыть замки
    digitalWrite(RELE_1, HIGH);
    digitalWrite(RELE_2, HIGH);
    }
  else {
